...
grid.arrange(cluster_plot,lda_plot,lsa_plot, nrow = 1, ncol = 3)
```
Sentiment Analysis:
...
train <- totdtm_ran[1:70000,]
train.labels <- totaltweet_random$isRetweet[1:70000]
test <- totdtm_ran[70001:100000,]
test.labels <- totaltweet_random$isRetweet[70001:100000]
...
table(nb_model, test.labels)
confusionMatrix(table(nb_pred, test.labels))
nb_cm <- confusionMatrix(table(ehr.pred, test.labels))
svm_model <- svm(train, train.labels, kernel = "linear")
