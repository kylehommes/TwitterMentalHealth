...
ggplot(hashtag_word_clust) + geom_bar(aes(x = factor(cluster), 
  ...
  geom_test(aes(x = factor(cluster), label = ..count..),
  stat = "count", vjust = -1) + labs(x = "Cluster", 
  y = "Number of Tweets", title = "Number of Tweets per Cluster")
ggplot(doc_topics) + 
  geom_bar(aes(x = factor(Topic), fill = factor(Topic)), 
  show.legend = FALSE) + 
  geom_text(aes( x = factor(Topic), label = ..count..),
