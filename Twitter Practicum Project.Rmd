...
ggplot(lda_tweet_topics) + 
  ...
  title = "Number of Words most associated with Each LDA Topic")
```
...
ggplot(hashtag_word_clust) + geom_bar(aes(x = factor(cluster), 
...
Classification:
```{r}
totaltweet_random <- sample(totaltweet)
```
